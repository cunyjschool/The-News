...
*/
    ...
    class FlexiThemes_FeaturedPosts
    {
        ...
        function FlexiThemes_FeaturedPosts()
        {
            global $theme;
            $this->theme = $theme;
            $this->url = FLEXIPANEL_PLUGINS_URL . '/featuredposts';
            ...
            $this->theme->add_hook($this->defaults['hook'], array(&$this, 'display_featuredposts'), 1);
  
            if(is_admin()) {
                $this->flexipanel_options();
            }
        }
        ...
        function display_featuredposts()
        {
            if($this->enabled()) {
                ...
                if(file_exists(FLEXIPANEL_PLUGINS_DIR . '/featuredposts/template.php') ) {
                    require_once(FLEXIPANEL_PLUGINS_DIR . '/featuredposts/template.php');
                }
            }
        }
        ...
        function featuredposts_source()
        {
            ...
            foreach($featuredposts_sources as $key=>$val) {
                ...
                ?>
                <input type="radio" name="featuredposts_source" onclick="javascript:flexipanel_featuredposts_source('<?php echo $key; ?>');" value="<?php echo $key; ?>" <?php echo $featuredposts_sources_selected; ?> /> <?php echo $val; ?> &nbsp;
                <?php
            }
            ?>
                <script type="text/javascript">
                    function flexipanel_featuredposts_source(source)
                    {
                        $flexipaneljQ("#flexithemes_featuredposts_category").hide();
                        $flexipaneljQ("#flexithemes_featuredposts_posts").hide();
                        $flexipaneljQ("#flexithemes_featuredposts_pages").hide();
                        
                        $flexipaneljQ("#flexithemes_featuredposts_"+source+"").fadeIn();
                    }
                    jQuery(document).ready(function(){
                        flexipanel_featuredposts_source('<?php echo $get_featuredposts_source; ?>');
                    });
        ...
        function flexipanel_options()
        {
            $this->theme->admin_option('General Options', array('Featured Posts', false, 15), 
                'Featured Posts', 'featuredposts_enabled', 
                'checkboxes', $this->defaults['enabled_in'], 
                array('options' =>array('homepage'=>'Homepage', 'categories' => 'Category Pages', 'tags' => 'Tag Pages', 'archives' => 'Archive Pages', 'searches'=>'Search Results Pages'), 'help'=> 'Enable featured posts slideshow in:', 'display'=>'extended-top')
            );
            
            $this->theme->admin_option('General Options', 'Featured Posts', 
