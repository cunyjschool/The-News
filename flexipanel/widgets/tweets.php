...
$flexi_tweets_defaults = array(
    'title' => 'Recent Tweets',
    'number' => '5',
    'username' => 'FlexiThemes',
    'link_title' => 'true',
    'refresh' => '30',
    'bird' => 'true',
    'autolinks' => 'true',
...
$theme->options['widgets_options']['tweets'] = is_array($theme->options['widgets_options']['tweets'])
    ?  array_merge($flexi_tweets_defaults, $theme->options['widgets_options']['tweets'])
    : $flexi_tweets_defaults;
    
add_action('widgets_init', create_function('', 'return register_widget("FlexiTweets");'));

class FlexiTweets extends WP_Widget 
{
    function FlexiTweets() 
    {
        $widget_options = array('description' => __('Advanced widget for displaying the recent tweets', 'flexithemes') );
        $control_options = array( 'width' => 400);
		$this->WP_Widget('flexi_tweets', '&raquo; Flexi Tweets', $widget_options, $control_options);
    }
    ...
    function widget($args, $instance)
    {
        ...
        if (!is_wp_error( $twitterFeed ) ) {
            ...
            if($maxitems > 0) {
            ?>
                <ul class="wrap-widget"><li class="flexi-tweets">
                <?php  if ( $title ) {  ?> <h3 class="widget-title"><?php 
                                ...
                                <?php
                                    ...
                                    if ($instance['date']) { 
                                        ...
                                        if ( ( abs( time() - $time) ) < 86400 ) {
                                            $human_time = sprintf( __('%s ago', 'flexithemes'), human_time_diff( $time ) );
                                        } else {
                                            $human_time = date(__('F j, Y'), $time);
                                        }
                                        
                                        $output_msg = $output_msg . sprintf( __('%s'),' <a href="'.$item->get_permalink().'" class="flexi-tweets-time" title="' . date(__('h:i A F j, Y'), $time) . '" target="_blank">'.$human_time.'</a>' );
                                    }
        ...
        ?>
            <div class="fp-widget">
                <table width="100%">
                    ...
                    <tr>
                        <td class="fp-widget-label"><label for="<?php echo $this->get_field_id('username'); ?>">Twitter Username:</label></td>
                        <td class="fp-widget-content"><input class="widefat" id="<?php echo $this->get_field_id('username'); ?>" name="<?php echo $this->get_field_name('username'); ?>" type="text" value="<?php echo esc_attr($instance['username']); ?>" /></td>
                    </tr>
                    
                    <tr>
                        <td class="fp-widget-label">Display:</td>
                    ...
                    <tr>
                        <td class="fp-widget-label">Misc Options:</td>
                        <td class="fp-widget-content">
