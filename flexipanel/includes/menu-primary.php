<?php
    ...
    $this->admin_option(array('Menus', FLEXIPANEL_ADMIN_URL . '/images/icon-menus.png', 25), 'Primary Menu', 
    ...
    $this->admin_option('Menus', 'Primary Menu', 
    ...
    if($this->display('menu_primary')) {
        ...
        // Register
        register_nav_menu( 'primary',  __( 'Primary Menu', 'flexithemes' ) );
        
        // Display Hook
        $this->add_hook($this->options['menus']['menu-primary']['hook'], 'flexi_menu_primary_display');
        ...
        $this->custom_js(flexi_menu_primary_js());
    }
    ...
    function flexi_menu_primary_display()
    {
        global $theme;
        ?>
            <?php $theme->hook('menu_primary_before'); ?>
			<?php wp_nav_menu( 'depth=' . $theme->get_option('menu_primary_depth') . '&theme_location=' . $theme->options['menus']['menu-primary']['theme_location'] . '&container_class=' . $theme->options['menus']['menu-primary']['wrap_class'] . '&menu_class=' . $theme->options['menus']['menu-primary']['menu_class'] . '&fallback_cb=' . $theme->options['menus']['menu-primary']['fallback'] . ''); ?>
              <!--.primary menu--> 
            <?php $theme->hook('menu_primary_after'); ?>	
        <?php
    }
    
    function flexi_menu_primary_default()
    {
        global $theme;
        ?>
    ...
    function flexi_menu_primary_js()
    {
        global $theme;
